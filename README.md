...
Примечание: Перед первым запуском необходимо создать схему в базе данных с названием "crm" в кодировке "utf8". Вновь зарегистрированный пользователь не может авторизироваться(залогиниться) в системе до тех пор, пока он находится в списке "новых" сотрудников (см. п.3 "Меню"). После того, как он будет принят и переместится из "новых" в список сотрудников, он сможет логиниться. "Меню" недоступно пользователям с ролью "USER". В базе данных за возможность логина в системе отвечают чекбоксы в таблице "user" в ячейке "is_enabled".
...
3 кнопка позволяет редактировать сотрудника, вторая его деактивирует с возможность активировать и 3 удаляет (нумерация в соответствии с номерами столбцов)
