...
public class IncomeStringToClient {
	...
	public static Client convert(String income) {
		Client client = null;
	...
	private static SocialNetwork getSocialNetwork(String link) {
		...
		if (link.startsWith("vk.com") || link.startsWith("m.vk.com")) {
			...
			socialNetwork.setSocialMarker(SocialNetwork.SocialMarker.VK);
		} else if (link.startsWith("www.facebook.com") || link.startsWith("m.facebook.com")) {
			socialNetwork.setLink(link);
			socialNetwork.setSocialMarker(SocialNetwork.SocialMarker.FACEBOOK);
		}
