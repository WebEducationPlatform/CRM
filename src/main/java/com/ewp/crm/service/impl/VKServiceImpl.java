...
public class VKServiceImpl implements VKService {
    ...
    public Client parseClientFromMessage(String message) throws ParseClientException {
        ...
        try {
            ...
            int number = 0;
            boolean flag = true;
            for (VkBid vkBid : vkBidService.getAll()) {
                if (flag) {
                    number = vkBid.getNumber();
                }
                if ("Обязательное".equals(vkBid.getType())) {
                    switch (vkBid.getName()) {
                        case "Имя":
                    ...
                    if (message.contains(vkBid.getName())) {
                        description.append(vkBid.getName()).append(": ").append(getValue(fields[number])).append(" \n");
                        flag = true;
                    } else {
                        flag = false;
                    }
