...
//                    client.deleteSocialProfile(socialProfile);
	...
	public void updateClient(Client client) {
		Client clientFromDB = get(client.getId());

		client.setWhatsappMessages(clientFromDB.getWhatsappMessages());
		client.setHistory(clientFromDB.getHistory());
		client.setComments(clientFromDB.getComments());
		client.setOwnerUser(clientFromDB.getOwnerUser());
		client.setStatus(clientFromDB.getStatus());
		client.setStudent(clientFromDB.getStudent());
		if (clientFromDB.getDateOfRegistration() == null) {
			setClientDateOfRegistrationByHistoryDate(client);
		} else {
			client.setDateOfRegistration(ZonedDateTime.parse(clientFromDB.getDateOfRegistration().toString()));
		}
		client.setSmsInfo(clientFromDB.getSmsInfo());
		client.setCanCall(clientFromDB.isCanCall());
		client.setCallRecords(clientFromDB.getCallRecords());
		client.setClientDescriptionComment(clientFromDB.getClientDescriptionComment());
		...
		List<String> emails = client.getClientEmails();
		List<String> phones = client.getClientPhones();
		...
		if (needUpdateClient) {
			clientRepository.saveAndFlush(clientFromDB);
		} else {
			clientRepository.saveAndFlush(client);
		}
	}
	...
	@Override
	public List<Client> getClientsByStatusIsLike(Status status) {
		return clientRepository.getClientsByStatusIsLike(status);
	}
}
